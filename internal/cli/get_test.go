...
func TestRunGetTranscriptLatestUsesStocksTranscriptValidation(t *testing.T) {
	var stdout, stderr bytes.Buffer

	code := runGet(context.Background(), []string{"transcript", "AAPL", "--latest"}, &stdout, &stderr, func(string) string { return "" })
...
func TestRunGetExposureUsesFundsExposureValidation(t *testing.T) {
	var stdout, stderr bytes.Buffer

	code := runGet(context.Background(), []string{"exposure", "AAPL"}, &stdout, &stderr, func(string) string { return "" })
