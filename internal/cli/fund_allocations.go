...
func runFundsCountryWeightings(ctx context.Context, args []string, stdout, stderr io.Writer, getenv getenvFunc) int {
	if len(args) == 0 {
		writeFundsCountryWeightingsHelp(stdout)
		return 0
	}
	if args[0] == "-h" || args[0] == "--help" || args[0] == "help" {
		writeFundsCountryWeightingsHelp(stdout)
	...
	if len(remaining) != 1 {
		fmt.Fprintln(stderr, "funds country-weightings requires exactly one symbol")
		return 2
	}
	...
	client := fmp.NewClient(apiKey, http.DefaultClient)
	weightings, err := client.ETFCountryWeightings(ctx, remaining[0])
	if err != nil {
		fmt.Fprintf(stderr, "funds country-weightings failed: %v\n", err)
		return 1
	}
...
func writeFundsCountryWeightingsHelp(w io.Writer) {
	fmt.Fprint(w, `Fetch ETF or fund country allocation weights.

Usage:
  stonk funds country-weightings <symbol> [flags]
...
func writeETFCountryWeightings(w io.Writer, weightings []fmp.ETFCountryWeighting, format outputFormat) error {
	switch format {
	case outputJSON:
		enc := json.NewEncoder(w)
		enc.SetIndent("", "  ")
		return enc.Encode(weightings)
	case outputCSV:
		return writeETFCountryWeightingsCSV(w, weightings)
	default:
		return writeETFCountryWeightingsTable(w, weightings)
	}
}

func writeETFCountryWeightingsTable(w io.Writer, weightings []fmp.ETFCountryWeighting) error {
	tw := tabwriter.NewWriter(w, 0, 0, 2, ' ', 0)
	fmt.Fprintln(tw, "COUNTRY\tWEIGHT")
	for _, weighting := range weightings {
		fmt.Fprintf(tw, "%s\t%s\n", weighting.Country, weighting.WeightPercentage)
	}
	return tw.Flush()
}

func writeETFCountryWeightingsCSV(w io.Writer, weightings []fmp.ETFCountryWeighting) error {
	cw := csv.NewWriter(w)
	if err := cw.Write([]string{"country", "weight_percent"}); err != nil {
		return err
	}
	for _, weighting := range weightings {
		if err := cw.Write([]string{
