...
func runGet(ctx context.Context, args []string, stdout, stderr io.Writer, getenv getenvFunc) int {
	...
	case "country-weightings":
		return runGetCountryWeightings(ctx, args[1:], stdout, stderr, getenv)
	...
	case "exposure":
		return runGetExposure(ctx, args[1:], stdout, stderr, getenv)
	...
	case "peers":
		return runGetPeers(ctx, args[1:], stdout, stderr, getenv)
	case "quote", "quotes":
	...
	case "sector-weightings":
		return runGetSectorWeightings(ctx, args[1:], stdout, stderr, getenv)
